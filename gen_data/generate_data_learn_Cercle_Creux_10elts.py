...
def create_inputs(nbElements):
   ...
   Rayon_ext = np.random.uniform(low=Length/100, high=Length/10)
   Rayon_int = np.random.uniform(low=Rayon_ext / 100, high= Rayon_ext / 1.1)
   ...
   d['r_ext'] = Rayon_ext
   d['r_int'] = Rayon_int
   d['S'] = (math.pi)*(pow(d.get('r_ext'),2)-pow(d.get('r_int'),2))
   d['I'] = (math.pi/4)*(pow(d.get('r_ext'),4)-(pow(d.get('r_int'),4)))

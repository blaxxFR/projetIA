...
def create_inputs(nbElements):
   ...
   Rayon = np.random.uniform(low=Length/100, high=Length/10)
   ...
   d['rho'] = rhoMat[iMat]
   d['r'] = Rayon
   d['S'] = (math.pi)*(pow(d.get('r'),2))
   d['I'] = (math.pi/4)*(pow(d.get('r'),4));
...
with open('test_Cercle_var.csv', 'w') as f:
    writer = csv.writer(f)
    header = ''
...
with open('dict_Cercle_var.csv', 'w') as csv_file:
